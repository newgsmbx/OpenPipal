...
/**
 * 规矩（hook）整体验收 —— 用户一句话定规矩，真 App 里从头到尾走一遍。
 *
 * 五段各有单测，但「模型照着 hook-creator 写出能加载的文件 → 加载器当场给结论 →
 * 对话流出现那行提醒 → 下一轮规矩真的拦住/改掉了工具结果 → 插件页能看能关」
...
const RULE_REQUEST = '以后读成绩表之前，先把学生的名字遮掉再给你看。这条规矩以后一直生效，不只是这一次。'
...
/** 隔离 home 跑完就删；把会话记录与规矩文件先抄到产物目录，失败了才有得看（不抄 config.json，那里有 key） */
...
test.describe('规矩整体验收（真模型）', () => {
  ...
  test('一句话定规矩 → 对话流提醒 → 下一轮真的遮了名字 → 插件页能关', async () => {
    ...
    await expect(notice, '对话流里没有出现「已定下规矩」胶囊（后台没写、或写完没送到渲染层）').toBeVisible({ timeout: 4 * 60_000 })
    ...
    say(`规矩文件：${hookFiles.join(', ') || '（无）'}`)
    expect(hookFiles.filter(f => /\.(ts|js|mjs|cjs)$/.test(f)), '后台没有把规矩写到约定目录').not.toHaveLength(0)
    await expect(notice).toContainText('已定下规矩')
    ...
    for (const name of NAMES) {
      expect(reply, `规矩没拦住：模型看到了真名 ${name}`).not.toContain(name)
    }

    // ---- 插件页：能看、能关；对话流那行随之变「已撤销」 ----
    await page.evaluate(() => (window as StoreWindow).__appStore?.getState().setActiveView('tools'))
    const rules = page.locator('[data-testid="plugin-rules"]').first()
    await expect(rules, '插件页没有列出规矩').toBeVisible({ timeout: 15_000 })
    ...
    expect(offFiles.some(f => f.endsWith('.off')), '关掉规矩应当是把文件改名成 .off').toBe(true)
