 ...
 * 规矩与宿主安全员的组合顺序（永久机制）。
 *
 * beforeToolCall：问答中断 → 规矩（改参 / 拦） → 安全员审**最终**参数。
 *   - 规矩改过的参数，安全员看到的就是改过的那份（同一个对象）
 *   - 规矩拦下的，安全员根本不会被调用（用户也就不会看到授权卡）
 *   - 没有规矩时行为与从前一致：直接走安全员
 * afterToolCall：宿主先算 terminate / isError，规矩再打补丁；宿主 terminate 不可覆盖，
...
describe('composePiCoreBeforeToolCall', () => {
  ...
  it('规矩拦下的调用也进审计', async () => {
    ...
    expect(security.writeHookBlockAudit).toHaveBeenCalledWith('bash', { command: 'rm -rf /' }, '被规矩「禁」拦下：不行')
  })

  it('规矩把参数改成不合 schema 的：拦下并说明，不让坏参数穿到工具里', async () => {
    const authorizer = new PiCoreToolAuthorizer({ conversationId: 'conv' })
  ...
  it('onToolStart 只在安全员放行后调用，规矩拦下或安全员拒绝都不调', async () => {
  ...
  it('规矩拦下的，安全员不会被调用，reason 带规矩描述', async () => {
    const authorizer = new PiCoreToolAuthorizer({ conversationId: 'conv' })
    const chain: HookChain = { hooks: [hook('no-net', { tool_call: [() => ({ block: true, reason: '考试日禁网' })] }, '考试日禁网')], ctx: ctx() }
    const run = composePiCoreBeforeToolCall({ authorizer, hookChain: chain, isInterrupted: () => false })
    await expect(run(beforeContext('web_search', { query: 'x' }), signal)).resolves.toEqual({ block: true, reason: '被规矩「考试日禁网」拦下：考试日禁网' })
    expect(security.authorizeToolCall).not.toHaveBeenCalled()
  })

  it('安全员拒绝的，规矩放行也没用', async () => {
    security.authorizeToolCall.mockResolvedValue({ block: true, reason: '越界' })
    const authorizer = new PiCoreToolAuthorizer({ conversationId: 'conv' })
...
describe('composePiCoreAfterToolCall', () => {
  it('没有规矩：宿主补丁原样返回，terminate 触发 onInterrupt', async () => {
  ...
  it('规矩改 content：模型看到的是改过的；宿主 isError 作为起点传给规矩', async () => {
  ...
  it('写文件探针：成功时把结论追加进工具结果，失败时不探', async () => {
    const probe = vi.fn(async (toolName: string, args: Record<string, unknown>) =>
      toolName === 'write' && args.path === '/p/hooks/x.ts' ? '【规矩已生效】x' : undefined)
    ...
    await expect(run(ok)).resolves.toEqual({ content: [{ type: 'text', text: 'written' }, { type: 'text', text: '【规矩已生效】x' }] })
  ...
  it('探针在规矩补丁之后：追加的那句话不会被规矩改掉', async () => {
    const chain: HookChain = {
