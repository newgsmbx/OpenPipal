...
const notice = { status: 'ok' as const, hookId: 'local-rules/mask', pluginName: 'local-rules', file: '/p/hooks/mask.ts', description: '读成绩表前先遮名字' }

describe('规矩提醒的落盘', () => {
  it('collector 把 hook_notice 收成独立条目，且不打断正文分段', () => {
    const collector = createTranscriptCollector()
    collector.feed({ type: 'text', content: '我来定规矩。' })
