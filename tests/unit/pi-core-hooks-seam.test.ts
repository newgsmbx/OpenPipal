/**
 * 规矩接线的源码级守卫——pi-core-runtime 必须经组合函数接 Agent，不能自己拼 beforeToolCall。
 *
 * 为什么按源码文本钉：组合顺序（规矩 → 安全员审最终参数；宿主 terminate 不可覆盖）住在
...
describe('pi-core-runtime 规矩接线', () => {
  ...
  it('规矩在工具装配之后装（借的就是这一轮的工具）；before_agent_start 在系统提示定稿之后、预算估算之前跑', () => {
    ...
    const chain = runtime.indexOf('const hookChain = await resolveHookChain(')
  ...
  it('规矩调工具走与助手同一份授权选项（conversationId / permissionHandler / scope / tier）', () => {
  ...
  it('规矩加载失败只留日志不影响对话', () => {
    expect(runtime).toMatch(/规矩加载异常，本轮不带规矩/)
