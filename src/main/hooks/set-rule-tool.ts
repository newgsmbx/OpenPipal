/**
 * set_rule：前台助手只递交"以后都要这样"的要求，写规矩文件的活在后台
...
export function createSetRuleTool(options: SetRuleToolOptions, deps: SetRuleToolDeps = { requestRule }): AgentTool {
  return {
    name: 'set_rule',
    label: '定规矩',
    description: `用户提出**以后长期生效**、且关于**你怎么用工具**的要求时调用："以后读成绩表先遮名字""别再跑 rm -rf""每次改完代码就跑测试""每轮开工先提醒我…"。你只递交要求，写规矩文件由后台完成，写好后 App 会直接提醒用户；调完立刻继续手头的任务。不要自己写规矩文件，不要说"已生效"。一次性的要求（"这次…"）不要调；教做事方法的写技能（skill-creator）。`,
    parameters: Type.Object({
      description: Type.String({ description: '一句大白话，照用户原话写，用户会看到这句（40 字以内）' }),
      details: Type.String({ description: '给后台写规矩的人看：什么时候触发（哪个工具、什么条件、涉及的路径/命令/关键词）、要做什么（改参数 / 拦下 / 改结果 / 每轮加什么提示）' })
    }),
    execute: async (_id, params) => {
      ...
      if (!description) {
        return { content: [{ type: 'text', text: 'error: description 为空——用一句大白话说清这条规矩' }], details: {} }
      }
      ...
      return {
        content: [{ type: 'text', text: `已交给后台写规矩「${description}」，写好后会直接提醒用户。继续当前任务；不要再自己写规矩文件，也不要说规矩已生效。` }],
        details: {}
      }
    }
  }
